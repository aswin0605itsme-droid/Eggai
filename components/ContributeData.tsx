...
const ContributeData: React.FC = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // e.g., await uploadForTraining({ image: imageFile, label: gender });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-gray-200 space-y-8">
        <Step number={1} title="Upload Egg Image">
            <label htmlFor="contribution-upload" className="flex flex-col items-center justify-center w-full h-56 border-2 border-amber-300 border-dashed rounded-lg cursor-pointer bg-amber-50 hover:bg-amber-100 transition-colors relative overflow-hidden group">
              {imagePreview ? (
        ...
        <Step number={2} title="Identify the Chick's Gender">
             <div className="grid grid-cols-2 gap-4">
                <button 
                    ...
                    className={`group flex flex-col items-center justify-center gap-2 py-4 rounded-lg border-2 transition-all duration-200 transform hover:-translate-y-1 ${gender === 'female' ? 'bg-pink-500 text-white border-pink-600 shadow-md' : 'bg-white hover:bg-pink-50 hover:border-pink-400 border-gray-300'}`}
                >
                ...
                <button 
                    ...
                    className={`group flex flex-col items-center justify-center gap-2 py-4 rounded-lg border-2 transition-all duration-200 transform hover:-translate-y-1 ${gender === 'male' ? 'bg-blue-500 text-white border-blue-600 shadow-md' : 'bg-white hover:bg-blue-50 hover:border-blue-400 border-gray-300'}`}
                >
        ...
        <Step number={3} title="Submit Your Data">
            <button
                ...
                className="w-full flex items-center justify-center gap-3 bg-amber-500 text-white font-bold py-4 px-4 rounded-lg hover:bg-amber-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105"
            >
