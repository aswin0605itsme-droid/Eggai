...
import { EggIcon, CameraIcon, ClipboardListIcon, BeakerIcon, DatabasePlusIcon, SearchIcon, ImageIcon, LogoIcon } from '../components/Icons';
...
export default function HomePage() {
  ...
  const TabButton = ({ tab, label, icon }: { tab: Tab; label: string; icon: React.ReactElement }) => (
    <button
      ...
      className={`group relative flex-1 flex flex-col items-center justify-center p-3 sm:p-4 text-xs sm:text-sm font-semibold transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 ${
        activeTab === tab
    ...
    >
      {icon}
      <span className="mt-1.5 hidden sm:inline">{label}</span>
      {activeTab === tab && (
